...
public class Mapa {
    ...
    public void colocarSoldados(Ejercito ejercito) {
    ...
    public void eliminarSoldadoDelTablero(Soldado soldado, Ejercito ejercito) {
        for (int i = 0; i < mapa.length; i++) {
            for (int j = 0; j < mapa[i].length; j++) {
                if (mapa[i][j] == soldado) {
                    mapa[i][j] = null;
                    ...
                    System.out.println("Soldado " + soldado.mostrarREF() + " eliminado de la lista de soldados de " + ejercito.getNombre());
                    return;
                }
            }
        }
