...
public class VideoJuegoMC {
			...
			while (true) {
			    ...
			    if (menu == 1) {
			        ...
			        while (true) {
			            ...
			            switch (sm) {
			                case 1: 
			                    if (ejerSel.size() < Soldado.MAXIMO_SOLDADOS) {
	...
	static int seleccionarEjercito(ArrayList<Soldado> e1, ArrayList<Soldado> e2) {
	...
	static int comprobarSeleccion(int min, int may) {		
		while(true) {
			int val=sc.nextInt();
			sc.nextLine();
			if(val<min || val>may) {
	...
	static void empezarJuego(Soldado [][] tab, ArrayList<Soldado> e1, ArrayList<Soldado> e2) {
		...
		System.out.println("TOTAL DE SOLDADOS: "+Soldado.totalSoldados());
		cantidadEjercito();
	...
	static void eliminarSoldados(ArrayList <Soldado> e) {
		for(int i=0; i<e.size(); i++) {
			if(!e.get(i).estaVivo()) {
				e.remove(i);
			}
	...
	static void cantidadEjercito() {
		System.out.println("SOLDADOS EJÉRCITO 1: "+Soldado.soldadosEjercito1());
		System.out.println("SOLDADOS EJÉRCITO 2: "+Soldado.soldadosEjercito2());
	}
}
